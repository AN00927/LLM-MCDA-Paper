...
| `Scenario Files/TestScenarios.xlsx` | `TestScenarios` | `decision_type`, `question`, `location`, `insulation`, `housing_type` | `square_footage`, `household_size`, `utility_budget`, `outdoor_temp`, `hvac_age`, `alternative_1`, `alternative_2`, `alternative_3` | none | `appliance_age` and `flow_rate` are empty placeholders; preserve them as blank optional columns if they remain. |
