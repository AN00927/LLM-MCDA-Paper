...
\usepackage{tikz}
%% FIX: 'shapes.rounded' is not a valid tikz library name; use 'shapes' instead
\usetikzlibrary{shapes,arrows.meta,positioning,fit,backgrounds,calc}
%% FIX: todonotes requires marginparwidth >= 2cm
...
- Describe what each calculator computes (energy cost in \$, environmental impact in lbs CO$_2$, comfort, practicality)
...
A.3 Shower Calculator (water heating energy, GPM $\times$ duration)
...
- Single region (Pennsylvania): emissions factor 0.6458 lbs
    CO$_2$/kWh and PPL TOU structure --- results may not generalize
...
\clearpage
\clearpage
%% FIX: \bibliographystyle and \bibliography must appear outside
%% \begin{thebibliography}...\end{thebibliography}, not inside it.
\bibliographystyle{elsarticle-num}
...
%% FIX: replaced \ifnum\coordindex conditional inside anchor={} with a
%% static anchor=north, which is valid for mostly-negative Pure values.
%% The \ifnum conditional caused "Unknown function 'north'" in PGF math.
\addplot+[
