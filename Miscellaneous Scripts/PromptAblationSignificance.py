...
Design:
  ...
  - Scenario-level metrics are averaged over the 5 runs first, giving one
    ...
    so effect estimates are comparable across the paper.
...
def main():
    ...
    print()
    print("=== FRIEDMAN (per architecture x model) ===")
    if len(friedman):
    ...
    print()
    print(f"=== SIGNIFICANT PAIRS (Holm-corrected): {len(sig)} of {len(posthoc)} ===")
    if len(sig):
